...
logging.basicConfig(
    format='%(asctime)s | %(levelname)s: %(message)s : %(value)s', level=logging.NOTSET)
...
def connectDB(coll_name):
    ...
    HOST = os.getenv('RASA_MONGODB_HOST')
    PORT = os.getenv('RASA_MONGODB_PORT')
    USERNAME = os.getenv('RASA_MONGODB_USERNAME')
    PASSWORD = os.getenv('RASA_MONGODB_PASSWORD')
    DB = os.getenv('RASA_MONGODB_DB')
    try:
        client = ""
        if (RUN_ENV == "local"):
            logging.info("Running in " + str(RUN_ENV))
        ...
        elif (RUN_ENV == "prod" or RUN_ENV == "dev"):
            logging.info("Running in " + str(RUN_ENV))
            logging.info("Host:" + str(HOST))
            logging.info("Port:" + str(PORT))
            logging.info("DB:" + str(DB))
        ...
        logging.error("Database connection problem: " + str(e))
...
def getResponse(response_name):
    ...
    try:
        logging.info("Looking for:" + response_name)
        res = my_coll.find_one({"response_name": response_name})

        if (res == None):

            logging.error("Result None for response name: " +
                          str(response_name))
        ...
        logging.error("MongoDB Operational Failure: " + str(e.details))
